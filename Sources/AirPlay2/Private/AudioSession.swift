...
class AudioSession {
    ...
    func start() {
        createAudioQueue(for: playerState, callbackQueue: callbackQueue) { [weak self] (queue, buffer) in
        ...
        createAudioBuffers(for: playerState.queue, count: playerState.bufferCount)
    }
    ...
    func setSequenceNumber(_ sequenceNumber: UInt16) {
        callbackQueue.async { [weak self] in
            ...
            self?.playerState.packets = [RTPPacket](repeating: EmptyPacket.packet(), count: 1024)
        }
    }
    ...
    private func createAudioQueue(for playerState: PlayerState, callbackQueue: DispatchQueue, callback: @escaping AudioQueueCallback) {
        var format = createFormat()
        var queue: AudioQueueRef? = nil
        AudioQueueNewOutputWithDispatchQueue(&queue, &format, 0, callbackQueue, callback)
    ...
    private func setMagicCookie(for queue: AudioQueueRef) {
        ...
        _ = magicCookie.withUnsafeBytes {
            AudioQueueSetProperty(queue, kAudioQueueProperty_MagicCookie, $0, UInt32(magicCookie.count))
        }
    }
    ...
    private func createAudioBuffers(for queue: AudioQueueRef, count: Int) {
        ...
        for _ in 0..<count {
            ...
            AudioQueueAllocateBufferWithPacketDescriptions(queue, maxPacketSize, numberOfPacketDescriptions, &buffer)
    ...
    private func loadBuffers() {
        for _ in 0..<playerState.buffers.count {
            ...
            handleOutputBuffer(playerState: playerState, queue: playerState.queue, buffer: buffer)
        }
    }
    ...
    private func handleOutputBuffer(playerState: PlayerState, queue: AudioQueueRef, buffer: AudioQueueBufferRef) {
        var packetCount = 0
        var offset = 0

        while hasAvailablePackets && manager.isPlaying {
            let packet = currentPacket
            if !checkPacketIsSequential(packet) {
                continue
            }

            if !checkBufferHasSpace(for: packet, atOffset: offset) {
                break
            }
    ...
    private func checkBufferHasSpace(for packet: RTPPacket, atOffset offset: Int) -> Bool {
        let packetSize = packet.payloadData.count
        return offset + packetSize <= playerState.bufferSize
    }

    private func writePacket(_ packet: RTPPacket, to buffer: AudioQueueBufferRef, index: Int, offset: Int) {
        let packetSize = packet.payloadData.count
    ...
    private func enqueueBuffer(_ buffer: AudioQueueBufferRef, to queue: AudioQueueRef) {
    ...
    private func handleEnqueueError(for buffer: AudioQueueBufferRef, in queue: AudioQueueRef, errorCode: Int? = nil) {
